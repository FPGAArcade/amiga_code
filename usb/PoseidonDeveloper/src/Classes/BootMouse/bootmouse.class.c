                              ...
                              a0, struct PsdInterface *, pif)
{
    ...
    if(ps = OpenLibrary("poseidon.library", 2))
    {
...
#define	SysBase	nch->nch_SysBase
    ...
    do
    {
        if(!(nch->nch_Base = OpenLibrary("poseidon.library", 2)))
        {
            Alert(AG_OpenLib);
            break;
        }
...
LONG nOpenBindingCfgWindow(struct NepHidBase *nh, struct NepClassHid *nch)
{
    ...
    if(!(PsdBase = OpenLibrary("poseidon.library", 2)))
    {
        return(FALSE);
    }
    Forbid();
...
void nGUITaskCleanup(struct NepClassHid *nch)
{
    ...
    if(MUIMasterBase)
    {
        ...
        MUIMasterBase = NULL;
#else
        if(!(--nch->nch_ClsBase->nh_MUIOpenCnt))
        {
            MUIMasterBase = NULL;
        }
#endif
