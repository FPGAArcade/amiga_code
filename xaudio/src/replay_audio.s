 ...
 * replay (private) ahiac_DriverData points to this structure.
	...
	ULONG	r_MMUFlags			; original mmu flags for XAUDIO buffer
...
;	ext.l	d0
	...
	bsr	SetMMU
	move.l	d0,r_MMUFlags(a3)

	kprintf	"Output (DMA) buffer MMU flags = $%lx",r_MMUFlags(a3)
...
;	CacheClearE(address:a0,length:d0,caches:d1)
		...
		moveq.l	#-1,d0
		cmp.l	r_MMUFlags(a4),d0
		bne.b	.skip

		move.l	r_OutputBufferAligned(a4),a0
		move.l	r_OutputBufferSize(a4),d0
		move.l	#CACRF_ClearD,d1
		CALLLIB	_LVOCacheClearE
.skip
		movem.l	(sp)+,d0-a6
		rts
