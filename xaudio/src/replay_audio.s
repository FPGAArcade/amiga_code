...
; TODO:
;	* add mono/stereo/hifi modes -> decode tags in AllocAudio -> add convert steps in CopySamples
;	* verify AHI calling notes
;	* calc interval based on freq
;	* calc output buffer based on freq/sample rate
 ...
 * replay (private) ahiac_DriverData points to this structure.
	...
	ULONG	r_EClock			; Currently not used (old DMA)
	ULONG	r_EPeriod			; Currently not used (old DMA)
	STRUCT	r_EAlarm,EV_SIZE		; Currently not used (old DMA)
...
.initRoutine:	; ( libptr:d0 seglist:a0 )
		...
		bsr.w	GetBoardAddr
		kprintf	"Replay xaudio BaseAddr = %lx",d0
...
PrintAudioCtrl:
	...
	kprintf "- AHIAudioCtrlDrv.ahiac_AntiClickSamples = %lx", ahiac_AntiClickSamples(a2)
	kprintf "- AHIAudioCtrlDrv.ahiac_PreTimerFunc     = %lx", ahiac_PreTimerFunc(a2)
	kprintf "- AHIAudioCtrlDrv.ahiac_PostTimerFunc    = %lx", ahiac_PostTimerFunc(a2)
	movem.l	(sp)+,d0/a0
	rts
...
AHIsub_Update:
	...
	;
	; Update TimerInt PlayerFreq (currently hardcoded to 20ms), and mix buffer size
	;
...
AHIsub_Stop:
	...
	movem.l	d0,-(sp)		; change to move.l
	btst	#AHISB_PLAY,d0
	beq.b	.dontplay
...
.dontplay
	movem.l	(sp)+,d0		; change to move.l
...
BeginIO:
	...
	movem.l	a6,-(sp)	; change to move.l
	move.l	IO_DEVICE(a1),a6
	jsr	-30(a6)
	movem.l	(sp)+,a6	; change to move.l
	rts

MICRO_DELAY = 20000		; hardcoded 20ms / 50fps
...
;		clr.b   LN_PRI(a2)
		...
		move.l  #MICRO_DELAY,IOTV_TIME+TV_MICRO(a1)
		move.l  a6,-(sp)
...
;	a4	READ BUFFER END
		...
		move.l	r_ReplayBase(a3),a5
		move.l	rb_BoardAddr(a5),a5

;	kprintf	"SRCPTR = %lx",XAUD_SRCPTRH(a5)
		tst.l	a5
		beq.b	.noreplay

		move.l	XAUD_SRCPTRH(a5),r_OutputGetPtr(a3)
...
;	a3	READ PTR
		...
		bsr.w	ec_readconv
		move.l	refills(pc),a0
;	kprintf	"FillOutputBuffer = %ld ms, with %ld refills",d1,a0

;		move.l	r_ReplayBase(a3),a0
;		move.l	rb_BoardAddr(a0),a0
;		move.l	XAUD_SRCPTRH(a0),d0
;	kprintf	"Current GET vs old GET = %lx vs %lx",d0,r_OutputGetPtr(a3)
...
;	subq.l	#1,d2
		beq.b	.notwrap
.copy
		cmp.l	a4,a3		; reached end of source?
		blo.w	.notend

		bsr.b	RefillBuffer

.notend
	; Assume 16 bit stereo source
		move.w	(a3)+,d6	; left
		move.w	(a3)+,d7	; right
...
;	asl.w	#1,d6
;	asl.w	#1,d7
	; convert data if not matching output here
...
.nomixerfunc
		...
		lea	.mixbuffer(pc),a5
		move.l	(a5),a3			; READ PTR
		lea	.mixbufferend(pc),a5
		move.l	(a5),a4			; READ BUFFER END
